...
called Chao's estimator~\cite{chao2016nonparametric} that was used to crack
the Enigma code during second world war~\cite{fuzzingbook2023:WhenToStopFuzzing}.
At its core lies the idea that the relative frequency of coverage of software
...
\begin{description}
  \item[RQ2] Are frequency based estimators affected by sampling strategies?
\end{description}

To evaluate the impact of different sampling strategies, we used three different
test suites---developer written, random generated, and guided. Our hypothesis
was that, if the estimators are indeed robust, then the estimations should
coincide. If on the other hand, some of the sampling strategies were brittle,
...
\begin{description}
  \item[RQ3] Are frequency based estimators affected by sampling units? % class and unit
\end{description}
