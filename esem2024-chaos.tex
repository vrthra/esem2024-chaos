...
the frequency based estimation methods provide an accurate
...
%\begin{displaymath}
%    S_{\textit{jack1}} = S_{\Unless stated otherwise, we reportextit{obs}} + Q_1\left(\frac{T-1}{T}\right); S_{\textit{jack2}} = S_{\textit{obs}} + \frac{2T-3}{T}Q_1 - \frac{(T-2)^2}{T(T-1)}Q_2.
%\end{displaymath}
...
Since our methodology required expensive manual mutant classification, %, which is %notoriously
%, we had to restrict the scope of our study; hence,
we restricted our analysis 
%as test subjects the
to \emph{ten largest projects} (\Cref{tbl:fullsubjects}, \Cref{tbl:subjectmeasures}) fulfilling our
criteria.
%We highlight these projects in \Cref{tbl:fullsubjects} and
%The main prsummarize their main properties in \Cref{tbl:subjectmeasures}.
...
We estimated the live mutant species richness
on the \projectCount test subjects using the \estimatorCount 
estimators on each of the three test suites using both test class and test method as sampling units.
\todo{
...
\end{subfigure}
\caption{Killable mutants estimated by frequency estimators for \Chao estimator, comparison with ground truth.
...
% i.e., ${|E - E^M|}$. 
%
\todo{ ALESSIO: I do not think we compute the Normalized MD, but the absolute one. Also, }
%
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|r|r|r|r|r|}
        \hline
        Estimator & Valid Est. & CI Overlaps & Mean (\%) & Stdev (\%) \\
        \hline
...
\todo{ALESSIO: This does not read really professional}
...
%
\todo{ALESSIO: We mention already Cohen's Kappa for the classification}
%
