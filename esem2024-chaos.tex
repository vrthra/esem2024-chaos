...
(ii) in most of the cases ($11/12$), the statistical estimators were significantly different than manual estimates (CI Overlaps $\le 3$), and
(iii) their mean difference show extreme variability (MD $\in (5.78 \pm 6.52, 19.76 \pm 20.51)$).
%
Focusing on the best performing statistical estimators, we can also observe that
\PCG, which achieved the maximum overlap with manual estimates, had overlapping CI with $E^M$ only in three out of \projectCount test subjects;
\ICEallrare, which achieved the smallest mean difference of 5.78 from $E^M$, 
...
%  had a large difference (19.76) from the manual estimate $E^M$.
...
their estimates from \original test suites were significantly different from
...
The figure shows that irrespective of test suite used---\original, \EvosuiteDynamosa, or \EvosuiteRandom---\Chao estimator's predictions are far from the ground truth.
