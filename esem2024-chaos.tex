...
\RQB showed that the estimators are affected by the adopted sampling strategy,
specifically by bias in sampling of killable mutants. \RQC, instead, focuses on the
...
or the (bad) results achieved in \RQA were a statistical fluke.
%
% As we mentioned in the introduction, there is a possibility, however remote,
% that the manual estimation result was either in error, or the result was a
% statistical fluke. To discount this possibility we conducted the third
...
We also observe that (1) test method estimators for most subjects seem to
overlap and have a smaller variance than class estimators, and (2) test method
estimators often (but not always) seem to provide an underestimate compared to
the manual estimates, while the same can't be said for class estimators.
(3) \Bootstrap seems to produce consistent estimates for method and class
estimators with small CIs when the number of mutants killed is far from total
generated mutants, while the uncertainties in CI increases drastically as the
killed mutants come close to total generated mutants. We observe the opposite
behavior in all other estimators. (4) In a significant number
of subjects, the difference between class and method estimators seem to
reduce the closer the number of killed mutants are to total generated.
In all these cases, the data is insufficient to clearly distinguish between
whether the observations are related to the total number of generated mutants
or the number of killable mutants because we found that in all \projectCount
subjects, the number of mutants were very small.
(5) Finally, we observe that the plots vary
considerably based on the subject in question. 
All the detailed plots
are available as Appendix in the replication package~\cite{replication-package}.
...
While there are several other enticing patterns in the data, the data we have
is insufficient to explore these fully. Hence, we will refrain
from interpreting or commenting on these until a larger follow up study can be
made.
...
Next, our analysis can be subject to bugs, sampling errors, and misclassification of mutants 
