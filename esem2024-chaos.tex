...
%
% ALESSIO: This part is not straightforward to follow as there's no connection between the sentences: we introduce equivalent mutant, and then jump to killable ones.
%			Maybe we can avoid mentioning equivalent mutant and say estimating how many mutants can be effectively killed is paramount to reliably estimated the adequacy of the tests?
%
Reliably detecting and eliminating killable mutants is difficult
as it is highly program and location dependent. 
%
% ALESSIO: The next sentence is also difficult to follow... why statistical estimation should offere a prosiming approach? what's statistical estimation? 
%			We introduce new terms and do not provide any argument. Maybe we can move the statistical estimation in the next "section"
%
Hence, statistical estimation of
killable mutants offers a promising approach.
% ALESSIO: This reads incomplete. An promising approach to what? 

\noindent\textbf{Aims.}
Recently, frequency based species estimation methods % ALESSIO: a widely used form of statistical estimation 
has been proposed as a solution for related problems in software testing.
%
% ALESSIO: the "related problems" feels a bit too generic. Maybe we can say esitmating residual bugs and establish the connection to mutation analysis?
%
...
%
% ALESSIO: Maybe restate WHY is this important? or How testers can use the results of this? 
%
%
...
% techniques.
Hence, this paper investigates the following: % ALESSIO: empirically investigate the following research question/hyphothesis (?)
\emph{Can
the species richness estimation methods provide an accurate % ALESSIO: before we call them species estiation, not species "richness" estimation. A bit confusing/inconsistent..
estimate for the number of killable mutants?}

\noindent\textbf{Method.} % ALESSIO: The text does not sound like a method. Maybe we can say something more about the empirical study ?
...
%
% ALESSIO: Probably we should add something for the future to make the message read more positive
%
%
...
a change in behavior in the mutant when compared to the original code.
...
are behaviorally \emph{equivalent} to the original code~\cite{budd1982two}.
%
% ALESSIO: Therefore they cannot be killed by any test.
%
%
...
%
% ALESSIO: Why so it is not necessary evident to everybody.
%			We might possibly say: highly variable, as equivalent mutants "inflates" it? 
%
...
\end{tcolorbox}
%
% ALESSIO: The box is misplaced, we should remove it. Better merge the text inside the box with the previous sentence.
%
...
%
% ALESSIO: "\emph{unseen} elements remaining for coverage evaluation." this wording is confusing. Does this mean coverage targets not covered/missed by a test suite? 
%
%
...
% predicting reachable coverage elements 
for predicting killable mutants.
%
%We find
%modeling of test cases or test classes as sampling units to be highly
%intuitive.
That is, we estimate the count of possibly killable mutants from the frequency of
mutant % kills
killed by test cases.
%
%
% ALESSIO: Also this box may create confusion. We should remove it and split the text around.
%			The idea of comparing single, double, etc. sampling is not introduced before. Also, it does not seem a relevant detail in this context.
%			If you think otherwise, probably we should introduce it when we firstly mention Chaos. 
%
\begin{tcolorbox}[boxrule=0.5pt, arc=4pt, boxsep=0pt, width=\columnwidth]
...
For answering RQ1, we first used \emph{test methods} as sampling units.
%
% ALESSIO: The above is uncalled for. And I wonder whether we should be so forthcoming with the Research Question.
%			The Introduction is rather long and we should probably make it more on point.
%
%\end{tcolorbox}
...
%
% ALESSIO: We say before those estimators are known to be robust towards sampling biases... studying this point feelds counter intutive.
%			We also have a footnote elaborating on this point...
%
%
...
test suites---developer written, randomly generated, and guided random to evaluate this question.
%
% ALESSIO: "guided random" is not a standard term to define dynamosa and the like. Why not using feedback-driven or search-based instead?
%
%
...
%
% ALESSIO: Starting the paragraph with another question to introduce yet another question does not read nice.
%		This RQ seems a sub-question of RQ2, why not presenting it like that? Further, we are interested in studying ...
%		The difference betweetn sampling strategies and sampling unit might be too subtle to appear that relevant. We can also say, sampling consists of two main dimensions: sampling strategy and granularity/unit. 
%		We investigate both...
%
...
%
%
%%%%%% ALESSIO: The following part does not seem to fit well in the introduction. It reads like a timeline of the study and some way to cover ourselves.
%			Instead of passing the message: we tried as much as possible, we might pass the message: the design was sloppy and they have to patch it.
%
%
...
% IMPORTANT: Not sure if we should include this because our tables do not have
% any thing to conclude this from.
...
point for a future investigationn on how to enhance the reliability of frequency
...
%
% ALESSIO: Possibly confusion/inconsitency: before we used the term Ecology (see the footnote)
%
These estimators
%, which have been used to estimate the size of unknown populations, 
have also been successful in counting systems in
computer networks~\cite{accettura2015the} and
estimating reachable coverage 
%in the STADS framework~
\cite{bohme2018stads}.
...
To make the paper self-containerd, the following sections introduce 
the fundational model underlying statistical estimation and the statistical
estimators our study considered.
...
%
% ALESSIO: why not also features of the code under test?
%
%
...
\]
where $W_{ij}=1$ if test $X_j$ kills mutant $M_i$, $0$ otherwise.
%$W_{ij} = 0$ otherwise.
...
coverage, % ALESSIO: Coverage is more a consequence, not a factor..
...
\normalsize
%
% ALESSIO: shouldn't the forall be OUTSIDE the probability?
%
...
any sampling unit.
The essential idea
%exploited by species richness estimators
is that the number
...
%\emph{singleton}
%species 
detected only once (\emph{singleton}), twice (\emph{doubleton}), %species detected only twice)
...
Gotelli and Chao~\cite{gotelli2013measuring} provided a version (referred as \<ICE-1>) for highly heterogeneous communities,
...
%
% ALESSIO: f1 and f2 undefined. Are they what we call Q1 and Q2?
%
%
...
%estimator and upper bound estimator with minor differences.
...
The remaining estimators we considered, namely \<\ChaoBunge>, \<UNPMLE>, \<PNPMLE>, and \<PCG>,
...
%Along with the incidence based estimators,
% There is a class of species richness estimators that work with abundance data.
Abundance sampling considers each individual to be a sampling unit
...
Besides, sometimes estimates assume Poisson rather than Binomial distribution of counts, which can be considered the limiting case when the number of tests tends to infinity
(i.e., $T$ is too big).
%
%Abundance sampling assumes independent random samples, but these estimators
%but as the sampling in biometrics is rarely such, the estimators based on this
%sampling
%are considered robust against violations of these assumptions~\cite{gotelli2011estimating}.
Having abundance data, we can apply
% Therefore, we additionally analyse 
\<\ChaoBunge>, \<UNPMLE>, \<PNPMLE>, and \<PCG> estimators.
...
%
Finally, the Poisson-compound Gamma model with smooth nonparametric maximum likelihood estimation (\<PCG>)
was provided by Wang et al.~\cite{wang2010estimating}.
...
% XX The comparison between \EvosuiteRandom and \EvosuiteDynamosa is in \Cref{tbl:estrandomdynamosamethod}.
...
% \hline
% \ICEallrare & 0 &  &  &  \\
% \Zelterman & 5 & 2 & 7.82 & 4.58 \\
% \ChaoBunge & 6 & 4 & 11.66 & 10.04 \\
% \Jackknife & 6 & 1 & 18.15 & 11.85 \\
% \Chao & 7 & 2 & 6.79 & 6.52 \\
% \improvedChao & 7 & 2 & 7.24 & 7.7 \\
% \ICE & 7 & 4 & 5.56 & 7.32 \\
% \improvedICE & 6 & 4 & 7.74 & 8.28 \\
% \Unpmle & 4 & 2 & 11.49 & 8.89 \\
% \Bootstrap & 7 & 3 & 16.19 & 27.53 \\
% \Pnpmle & 6 & 4 & 14.32 & 14.12 \\
% \PCG & 5 & 3 & 18.55 & 26.39 \\
% \hline
% \end{tabular}
...
%difference with respect to manual classification.
%
%The results are given in tables \Cref{tbl:estoriginalclass}, %-1-,
%\Cref{tbl:estrandomclass}, %-2-
%and \Cref{tbl:estdynamosaclass}.
...
%X \begin{table}[h]
%X     \caption{Comparison of \emph{mean difference (MD)} between class estimators across subjects---\original test suites}
%X     \begin{tabular}{|l|r|r|r|r|r|}
%X         \hline
%X         Estimator & Valid Estimates & CI Overlaps & Mean & Stdev \\
%X         \hline
%X         \ICEallrare & 4 & 1 & 6.26 & 9.77 \\
%X         \Zelterman & 5 & 1 & 9.33 & 9.26 \\
%X         \ChaoBunge & 4 & 0 & 9.64 & 10.86 \\
%X         \Jackknife & 4 & 0 & 7.75 & 9.96 \\
%X         \Chao & 5 & 1 & 14.89 & 10.35 \\
%X         \improvedChao & 4 & 0 & 14.2 & 9.28 \\
%X         \ICE & 4 & 0 & 16.54 & 10.51 \\
%X         \improvedICE & 4 & 0 & 12.21 & 9.45 \\
%X         \Unpmle & 3 & 0 & 11.14 & 9.63 \\
%X         \Bootstrap & 5 & 2 & 25.83 & 22.67 \\
%X         \PCG & 2 & 0 & 9.94 & 13.3 \\
%X         \hline
%X     \end{tabular}
%X     \label{tbl:estoriginalclass}
%X \end{table}
%X \begin{table}[h]
%X     \caption{Comparison of \emph{mean difference (MD)} between class estimators across subjects---\EvosuiteRandom test suites}
%X     \begin{tabular}{|l|r|r|r|r|r|}
%X         \hline
%X         Estimator & Valid Estimates & CI Overlaps & Mean & Stdev \\
%X         \hline
%X         \ICEallrare & 3 & 0 & 16.4 & 14.54 \\
%X         \Zelterman & 7 & 4 & 10.65 & 9.61 \\
%X         \ChaoBunge & 2 & 1 & 5.44 & 5.24 \\
%X         \Jackknife & 4 & 2 & 7.23 & 6.24 \\
%X         \Chao & 7 & 2 & 16.56 & 9.94 \\
%X         \improvedChao & 5 & 1 & 15.56 & 5.6 \\
%X         \ICE & 5 & 0 & 19.05 & 11.91 \\
%X         \improvedICE & 3 & 2 & 6.5 & 0.63 \\
%X         \Unpmle & 3 & 2 & 9.02 & 14.04 \\
%X         \Bootstrap & 9 & 1 & 33.76 & 22.22 \\
%X         \Pnpmle & 1 & 1 & 1.43 &  \\
%X         \PCG & 2 & 2 & 1.56 & 1.58 \\
%X         \hline
%X     \end{tabular}
%X     \label{tbl:estrandomclass}
%X \end{table}
%X \begin{table}[h]
%X     \caption{Comparison of \emph{mean difference (MD)} between class estimators across subjects---\EvosuiteDynamosa test suites}
%X     \begin{tabular}{|l|r|r|r|r|r|}
%X         \hline
%X         Estimator & Valid Estimates & CI Overlaps & Mean & Stdev \\
%X         \hline
%X         \ICEallrare & 1 & 1 & 0.73 &  \\
%X         \Zelterman & 3 & 0 & 24.96 & 21.6 \\
%X         \ChaoBunge & 2 & 1 & 4.19 & 0.3 \\
%X         \Jackknife & 1 & 0 & 37.27 &  \\
%X         \Chao & 4 & 0 & 23.86 & 21.07 \\
%X         \improvedChao & 4 & 0 & 19.15 & 20.24 \\
%X         \ICE & 4 & 1 & 21.01 & 21.93 \\
%X         \improvedICE & 3 & 1 & 15.47 & 14.95 \\
%X         \Bootstrap & 6 & 0 & 49.04 & 15.57 \\
%X         \Pnpmle & 1 & 0 & 1.49 &  \\
%X         \hline
%X     \end{tabular}
%X     \label{tbl:estdynamosaclass}
%X \end{table}
...
\ICEallrare achieves a smallest mean difference (MD) of 5.78 from $E^M$.
Finally, \Bootstrap achieves the
maximum number of \emph{valid estimates} (VE) with 9 valid estimates,
but has a much larger difference from the manual estimate $E^M$ (19.76).
...
The figure shows that irrespective of test suite used---\original, \EvosuiteDynamosa, or \EvosuiteRandom---\Chao estimator predictions are far from the ground truth.
...
%from manual classification when given unbiased test suites?
Are frequency based estimators affected by sampling strategies?}\\
As we noted in the introduction, it is possible that the frequency
based estimators may not be robust to bias in sampling.
To mitigate this bias, we used two generated test suites \EvosuiteRandom and
\EvosuiteDynamosa without manual bias.
%That is, when
%developers write tests, they choose which parts of the code to focus on.
%This may mean that the results from \original test suites may not be
%usable for estimation since the live mutants remaining may be affected
%by this bias. Hence, we used two more test suites with different test
...
% their 95\% confidence intervals overlap less than 50\% of the time
% (\Cref{tbl:estrandomdynamosamethod}).
...
We expected that using random test generation strategies would mitigate
manual bias, and hence result in better estimates. However, in our experiment,
we see that the mean difference observed from generated test suites is much
higher than that observed from the \original test suite.
...
From \Cref{tbl:estoriginalclassmethod}, \Cref{tbl:estrandomclassmethod}, and
and \Cref{tbl:estdynamosaclassmethod}, we find that the 
estimates from method level sampling units
and class level sampling units are statistically significantly different.
\Bootstrap had the highest (50\%) overlap for \EvosuiteRandom. However, for
other test suites, \Bootstrap did not produce a significant overlap of CI between
method and class estimators.
...
significant for every estimator examined.
...
While overall, the result is negative, we observe a glimmer of hope. We note
that in \Cref{tbl:estrandomclassmethod} comparing method and class sampling
...
may be killable by automatic test generators due to deficiencies in oracles
available. Hence, it is possible that \Bootstrap estimation is true to the
actual value, albeit with a large amount of uncertainty. 

Furthermore, unlike \EvosuiteDynamosa, \EvosuiteRandom is completely unguided in
