...
\begin{subfigure}{.29\textwidth}
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/3/Chao-dynamosa.pdf}
\vspace*{-3mm}
...
\end{subfigure}
\begin{subfigure}{.29\textwidth}
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/3/Chao-random.pdf}
\vspace*{-3mm}
...
estimate that resulted in $\infty$ in upper CI\todo{
We should also remove any that results in a point estimate above 100\%}. The count of such valid
...
\noindent\textbf{RQ1} \textbf{How accurate are frequency based estimators in predicting
    killable mutants?}\\
Inspecting \Cref{tbl:estoriginal}, we note that \Unpmle achieved a maximum of
50\% agreement (five out of \projectCount test subjects, which was the highest,
MD $11.1$\%, Stdev. 9.8\%.)\todo{
This needs to be verified. Currently, it seems that we have a smaller number of
valid estimates than 10, and their mean difference should be larger.
} However, the confidence
intervals were very large in many cases for
\Unpmle\todo{Add a reference to appendix where we provide the CI}.
For an estimator to be useful in predicting the killable mutants, it must be
able to both \emph{accurate} and \emph{precise}. \textbf{Accuracy} means that the
prediction must be close to the correct value, and \textbf{precision} means that
the uncertainty in prediction must be low.
While \Unpmle has moderate accuracy, its precision is very low.
...
% ADDED
To illustrate the differences, we plot the results for \Chao estimator\todo{
  We should do \Unpmle instead.} (the
...
estimator suggested by the STADS framework in
...
\begin{subfigure}{.29\textwidth}
    \centering
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/4/Chao-methods-dynamosa-random.pdf}
\vspace*{-3mm}
\caption{\textcolor{cdynamosa}\EvosuiteDynamosa vs \textcolor{crandom}\EvosuiteRandom}
\end{subfigure}
\begin{subfigure}{.29\textwidth}
    \centering
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/4/Chao-methods-organic-random.pdf}
\vspace*{-3mm}
...
\begin{subfigure}{.29\textwidth}
    \centering
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/4/Chao-classes-dynamosa-random.pdf}
\vspace*{-3mm}
\caption{\textcolor{cdynamosa}\EvosuiteDynamosa vs \textcolor{crandom}\EvosuiteRandom}
\end{subfigure}
\begin{subfigure}{.29\textwidth}
    \centering
      \includegraphics[width=0.95\linewidth,trim=8.6cm 0cm 0cm 0cm, clip=true]{chaoplots/4/Chao-classes-organic-random.pdf}
\vspace*{-3mm}
\caption{\textcolor{coriginal}\original vs \textcolor{crandom}\EvosuiteRandom}
\end{subfigure}
...
% estimates that are accurate and precise. \todo{Better ones after we shift to MAE}
% Previous work~\cite{papadakis2014mitigating} suggests that non-killable mutants
% range from 9\% to 20\% of all mutants generated. To be useful, an estimator
% should be able to consistently produce predictions that are more precise than
% this range.
% That is, the point estimate should be close enough
