...
was proposed for estimating the number of \emph{reachable} but yet-to-be-covered elements remaining for coverage evaluation.
...
The problem of estimating species richness,
is challenging because the geographical area
...
(2) randomly select sampling units to survey for number of species found.
...
However, we can consider the whole test suite as one single sample and count number of kills for each mutant.
...
that developers %manually
wrote to check application- and
domain- specific requirements.
Therefore, one can argue that using such \original test suites 
...
We leveraged the mutation testing framework \PIT~\cite{pit} (v~1.4.9)
to generate the mutants for each project
and executed all the available test suites against them. %all the generated mutants .
Since mutation analysis is almost always used with unit
tests in practice, we did not consider integration or system tests.
Nevertheless, this activity resulted in the execution of more than $2.5$B test methods.
%
...
the open-source infrastructure developed for the SBST Java unit test tool competition,
and extended it to address \PIT's limitations and heavily parallelize the test executions across multiple computing nodes (we used a cluster of 50 nodes featuring heterogeneous hardware configurations).
...
%
% ALESSIO: The following applies to all the test suite, so I moved where we talk about killable mutants, here I state we sample survided mutants only in the ORIGINAL test suite.
%
...
Specifically, we considered the mutants that survived the \emph{\original} test suites
by looking at those mutants that \PIT marked as \texttt{SURVIVED} or \texttt{NOT\_COVERED}.
...
% i.e., ${|E - E^M|}$. 
%
% ALESSIO: I do not think we compute the Normalized MD, but the absolute one. Also, 
%
Next, we computed the normalized
...
Notably, we removed any invalid estimate before computing MD, gaining $m$ valid estimates.
...
and statistics about MD (columns \emph{Mean} and \emph{Stdev}) aggregated across test subjects
...
To answer this questions, we follow the same approach adopted to answer RQ1.
...
against the corresponsing estimations using test classes as
sampling unit for all tests suites. 
% As before, we remove invalid estimates. ALESSIO: Do we?
The comparison between class and method estimators for \original is given
in \Cref{tbl:estoriginalclassmethod}.
That for \EvosuiteRandom is given in \Cref{tbl:estrandomclassmethod},
and for \EvosuiteDynamosa is given in \Cref{tbl:estdynamosaclassmethod}.
...
% ALESSIO: This does not read really professional
...
(i) none of the \estimatorCount statistical estimators generated a valid estimate (VE) for all the test subjects (VE $\in [4, 9]$);
(ii) in most of the cases ($11/12$), the estimations were significantly different than manual estimates (CI Overlaps $\le 3$), and
(iii) their mean difference showed extreme variability (MD $\in (5.78 \pm 6.52, 19.76 \pm 20.51)$).

%
% ALESSIO: I am not sure where the description of the figure fits... Somehow, it seems that tables are more than enough...
%
...
%
In the figure, the x-axis represents the percentage of mutants, with 1.0 (bold solid line) indicating the complete set of generated
mutants, whereas the y-axis lists all the test subjects.
For each test subject, the figure shows the percentage of killed mutants (black dotted line),
...
and \Bootstrap, which achieved the maximum number ($9$) of \emph{valid estimates},
...
In light of these observations, we cannot reliably say whether any of
the studied estimators might be useful to the developers in practice
or reccomend any of them to be used in mutation analysis.
%
...
%
%
% ALESSIO: This seems to restated the observations made above... 
%
...
Consequently, we answer RQ1 as follows:
\begin{tcolorbox}[boxrule=0.5pt, arc=4pt, boxsep=0pt, width=\columnwidth]
Despite some positive results, in general the \estimatorCount statistical
...
Statistical estimators rely on the data obtained by sampling campaigns;
...
\EvosuiteRandom and \EvosuiteDynamosa.
...
% ALESSIO: some lost, some gain, the majority stayed the same
and, (iii) the mean difference deagraded considerably ($+20\%$ on average).
% \EvosuiteRandom was considerably fewer than that between manual and \original.
% The smallest difference in MD was observed for \ICEallrare,
% (mean 19.51\%, Stdev. 21.08\%). However, \ICEallrare
% had just two \emph{valid estimates}.
...
reveals that the situation did not improve using an advanced test generation algorithm.
...
Instead, we plot in \Cref{fig:methodsamplingunit} the comparison of the sampling strategies.% and \Cref{fig:classsamplingunit}
%
% ALESSIO: Is this just reshuffling the content of the Figure 3? Do we really need it?
% ALESSIO: We should comment something about the figure...
%
...
otherwise.
%
Our expectations were met but only partially.
...
they should have produced larger confidence interval in the face of fewer samples.
In turn, large CIs would have increased the \emph{accuracy} of the predictions, i.e., the number of CI overlaps,
at the expense of their \emph{precision}, i.e., mean difference. 
But this was not the case; therefore, we answer RQ2 as follows:
\begin{tcolorbox}[boxrule=0.5pt, arc=4pt, boxsep=0pt, width=\columnwidth]
...
RQ2 showed that the estimators are affected by the adopted sampling strategy,
speficically by bias in sampling of killable mutants. RQ3, instead, focuses on the
...
we expect estimators to produce comparable results, as they ultimately
...
discount the possibility that the manual classification result was either in error,
or the (bad) results achieved in RQ1 were a statistical fluke.
%
% As we mentioned in the introduction, there is a possibility, however remote,
% that the manual estimation result was either in error, or the result was a
% statistical fluke. To discount this possibility we conducted the third
...
and \Cref{tbl:estdynamosaclassmethod}, we can observe that the 
estimates from method level sampling units and class level sampling units
are %statistically
...
This situation is clearly exemplified in figures
\ref{fig:groundtruth}, \ref{fig:groundtruth3}, and \ref{fig:groundtruth2}
for \Chao estimator, in which almost all the CI's for the same test subject
and sampling strategy do not overlap.

In light of these results, we answer RQ3:
\begin{tcolorbox}[boxrule=0.5pt, arc=4pt, boxsep=0pt, width=\columnwidth]
...
are statistically significant for every estimator examined. Consequently,
...
may be killable by automatic test generators due to techinical limitations or
