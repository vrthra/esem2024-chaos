...
the number of faults that are likely to remain in the software
after a testing campaign ends, helps in addressing important
questions about software quality (e.g.,  "How much can we trust
a software after testing it?") and automatic test generation
(e.g., "How effective is a test generator? How much should it improve
to produce reliable results?").
%
Traditionally, those questions are addressed using structural coverage metrics;
for instance during fuzzing, frequency based statistical models
for unknown software species have been
recently proposed for estimating \emph{reachable coverage}, 
a proxy for residual risk.

Mutation analysis with its ability to induce and evaluate simple faults
in structural elements of the code can provide a more reliable answer
than structural coverage and, provided one can estimate 
the \emph{number of killable mutants}, a more representative % ALESSIO: In the intro we say that ALIVE Mutants are a proxy for residual risk. Can we solve this inconsistency?
proxy for residual risk than reachable coverage.
...
statistical models for estimating the number of detectable mutants in
