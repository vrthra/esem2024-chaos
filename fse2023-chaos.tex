...
\end{comment}
\begin{abstract}
A testing professional may often need to provide guarantees in the robustness
of the software after a testing campaign has concluded. How much more can the
client trust the software?
This can be accomplished by estimating the residual risk --- the upper-bound on
the number of faults that are likely to remain in the software.
...
For both these cases, mutation analysis can provide a more reliable answer than
structural coverage measures due to its ability to induce and evaluate all
possible simple faults in structural elements of the code.
...
we conducted on the application of \estimatorCount widely known frequency based
...
An accurate estimate of the killable mutants is important for evaluating the effectiveness
of test suites as well as evaluating residual risk after a test run, both of which are important
problems for a practitioner.
\end{tcolorbox}

The main problem is that the number of killable %equivalent
...
for \projectCount  mature and well-tested open-source projects (\Cref{tbl:subjectmeasures}). % from Apache Commons. %mature and
...
That is, their estimates were significantly different from the manual estimates.
