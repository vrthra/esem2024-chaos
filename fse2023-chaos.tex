...
Live mutants can be also considered as
...
Unfortunately, not all the live mutants could be killed. For instance, the
...
Recently, researchers proposed to adopt the Software Testing as Species Discovery (STADS) framework~\cite{bohme2018assurances}
...
While STADS is initially proposed for estimating reachable coverage,
we argue %observed 
that the same model is applicable for estimating live mutants % to mutation analysis
...
Therefore, in light of those observations, % and considering their robustness towards bias in sampling,
%
%we wondered
...
existing test suites and used this results for assessing the prediction quality of
...
Frequency based estimators have been used extensively in biometrics (Ecology)
...
Estimators \<UNPMLE>, \<PNPMLE>, and \<PCG>
...
\label{sec:mapping}
Estimating the number of killable mutants can also be
done through population estimation. Here,
each killable
...
detected by a single test case (singletons), detected by
two test cases, etc. 
%
...
(1) We identified manual classification as the most important part.
...
in different computing clusters, and the results to be easily reproducible.
...
\begin{enumerate*}[label=(\arabic*)]
  ...
  \item packaged as a single Maven module.
\end{enumerate*}
...
17 projects that have a complex structure or fail to build or do not pass the available tests or break mutation analysis even after some effort into fixing them.%, although we put some effort into fixing existing problems.

Since our methodology requires manual live mutant classification, which is %notoriously
...
%
To reduce the risk of misclassifying live-but-slow mutants as killed,
...
is not applicable to the problem of estimating killable mutants.
...
the residual risk, and for estimating the effectiveness of a test generator.
While a sound classifier for killable and equivalent mutants is impossible,
recent advances in statistical estimation using frequency count based 
estimators gave hope that one could at least estimate the number of such mutants.
Such estimators have been used successfully in related domain such as reachable
coverage estimation, and estimating reachable computer systems in a network. 
...
the killable mutants esimation do not produce consistent results across
