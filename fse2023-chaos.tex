...
overestimation of redidual risk.
...
Estimating residual risk, i.e., the upper-bound on
the number of faults that are likely to remain in the software
after a testing campaign, helps in addressing important
questions about software quality, (e.g., \emph{``How much can we trust
a software after testing it?''}), and automatic test generation
...
faults in structural elements of the code can provide a more
reliable answer than structural coverage and, provided one can accurately
estimate the \emph{number of live but killable mutants}, a more
representative proxy for residual risk than unseen but reachable coverage.
...
%
Specifically, mutants are copies of the original code (\Cref{fig:working-example}-left) in which artificial, %simple, 
but plausible faults that share strong similarities with real faults are injected (\Cref{fig:working-example-mutants})~\cite{daran1996software,just2014are,andrews2005is,andrews2006using}.
...
(see \Cref{fig:working-example-mutants}-right for
...
the number of killable mutants (see~\Cref{eq:1}) but cannot be established 
...
%\footnote{Software Testing as Species Discovery} 
as a general framework for estimating %the 
\emph{unseen} software % ALESSIO: Can we say "unobserved/unobservable" instead?
elements that fulfill a property when only a fraction of those software elements has been discovered.
% ALESSIO: Hard to follow. Maybe moving the example here helps?
...
software elements % of the same kind that is
yet to be observed.
%
...
% counts, where
%the species counts of different species are often highly correlated and geography specific. 
% to treat software elements that are currently covered as observed entities, and use the frequency of their observation (coverage by specific test cases) to predict how many structural
...
we argue %observed 
that the same model is applicable for estimating surviving killable mutants % to mutation analysis
because covering a structural element is a prerequisite for detecting any
mutation in that element. 
%Therefore, given a test execution, %the event of 
%covering structural elements in the code can be mapped to %the event of 
%detecting mutants affecting it. %the same code locations.
% ALESSIO: Maybe we can executing a "a structural element" --- "covering/stressing" a mutant?
%
%Furthermore, mutation analysis is quite close to structural coverage
%as covering a structural element is a prerequisite for detecting any mutation in that element.
In the light of this observation, % and considering their robustness towards bias in sampling,
%
%we wondered
...
%
% It was recently
%proposed to adopt STADS framework to estimate the total number of killable
%mutants using the distribution of already killed mutants~\cite{gopinath2022mutation}.
%
%Specifically, the killable mutants are assumed to belong to a
%\emph{population}, or \emph{species}, whose size can be 
%estimated by means of existing statistical estimators
%(e.g.,~\cite{chao2016nonparametric,bohme2018assurances}).
%
%However, those estimators rely on assumptions
%about the distribution of the population and behave differently when 
%such assumptions do not hold, and their ability to
%predict the number of killable mutants has not yet 
%empirically evaluated.
%
%To study this,
% whether such frequency analysis based statistical models can estimate the number
%of killable mutants better, 
...
for \projectCount  mature, well-tested open-source projects. % (\Cref{tbl:subjectmeasures}). % from Apache Commons. %mature and
This paper summarizes 
the main findings of our study and is complemented by
the replication package~\cite{replication-package}.
%and we publish our replication package
% while the attached
%replication package
% contains the whole dataset, results, 
...
using PIT, we manually classified 1016 live mutants that survived those open-source projects'
existing test suites and used this result for assessing the prediction quality of
...
%
To mitigate the second threat, instead, we compared the estimates from \Evosuite
...
We, again, found our expectations unmet as manual classification was significantly different from
...
to estimate the size of an unknown population. Besides, they were also suggested for use in the
...
first introduce the underlying model and then describe various statistical estimators that we considered in our study.

\subsection{Statistical Model}
At first, let's consider the following well known statistical model:
Suppose, we have an urn with colored balls, from which $n$ balls are sampled with replacement and $S(n)$ colors are observed.
...
In application to our problem, this urn sampling, which is described with the Bernoulli product model, associates each test (ball) with the killed mutants (colors of a ball).% or `species`
...
Let $\{M_i\}_{i=1}^S$ be a set of mutants, and a mutant ${M_i}$ can be detected with a probability $\pi_i$.
The detection rate of a mutant might be affected by factors specific to its definition (e.g.,
...
For a testing campaign of size $N$, we let the \emph{kill incidence} matrix, or simply killmatrix, $W_{S\times T}$ be defined as
\[
...
This way, the $i_{th}$-row sum of $W$ ($Y_i=\sum_{j=1}^{T}W_{i,j}$) denotes the incidence-based frequency of mutant $M_i$.
We further define the incidence frequency counts $Q_k$, where $0 \leq k \leq N$, as the number of mutants killed by exactly $k$ tests.
Then, the unobservable frequency count $Q_0$ denotes the number of mutants that remain undetected.

We assume that the probability that a mutant~$i$ is detected by a test~$j$ is defined as
...
We model it as a random variable from an unknown probability density function $h(\nu)$; whereas we assume that the mutant detection rates $\pi_i$ are fixed.

Consequently, we can model the probability distribution of each element $W_{ij}$ of the killmatrix as a Bernoulli random variable conditioned on $\nu_j$:
\small
\begin{displaymath}
    P(\forall (i,j).W_{ij}=w_{ij}|\nu_j)=(\pi_i \nu_j)^{w_{ij}}(1-\pi_i \nu_j)^{1-w_{ij}}.
\end{displaymath}
\normalsize
So as the probability distribution for the incidence matrix can be expressed as the probability for all $i:1\leq i \leq S $ and $j:1\leq i\leq T$ that we have $W_{ij} = w_{ij}$.
\[
    P\left(\forall (i,j).W_{ij}=w_{ij}|\nu_{i} \right)= \prod_{j=1}^{T}\prod_{i=1}^{S} {\pi_i \nu_j}^{w_{ij}} (1-p_i)^{1-w_{ij}}
\]
Integrating out all possible values of $\left\{\nu_1, \nu_2,\cdots,\nu_T \right\}$, we obtain the following unconditional marginal distribution for the incidence-based frequency $Y_i$ for the mutant $i$,
...
Then the incidence frequency counts can be derived as
\[
...
(1) they divide the area into \emph{sampling units}; %, that can be surveyed comprehensively; 
%
(2) randomly select %choose a %smal
...
and the number species found. % that comprise to them.
%
...
\hline
\multicolumn{1}{|c|}{Project} & \multicolumn{1}{c|}{Sampled} & \multicolumn{2}{|c|}{Miss. Rate} & \multicolumn{1}{c|}{Equivalent}\\
    & & \rB & \rC & \\
\hline
...
number of killable mutants in a project using biased (RQ1) and unbiased
(RQ2) test suites as well as the robustness of such estimators against
changes in sampling strategy (RQ3).
...
estimates from manual classification % (i.e., the ground truth)
...
our (current) conclusion is that frequency based statistical %standard species richness 
...
Accurately estimating the number of killable mutants is crucial for both estimating
